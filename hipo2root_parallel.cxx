...
// --- Your original per-file conversion logic, unchanged, just wrapped in a function ---
void processFile(const TString& inputDir, const TString& inputFile,
...
// --- New: derive "name.root" from "name.hipo" ---
...
int main(int argc, char** argv) {
    ...
    bool saveMC        = std::stoi(argv[3]) != 0;
    int njobs          = std::stoi(argv[4]);
    ...
    // Split the file list round-robin across njobs workers
    std::vector<std::vector<std::string>> chunks(njobs);
    for (size_t i = 0; i < inputFiles.size(); i++) {
        chunks[i % njobs].push_back(inputFiles[i]);
    }

    std::vector<pid_t> children;
    for (int j = 0; j < njobs; j++) {
        if (chunks[j].empty()) continue;

        pid_t pid = fork();
        if (pid < 0) {
            std::cerr << "fork() failed for worker " << j << std::endl;
            continue;
        }

        if (pid == 0) {
            // Child: process this worker's assigned files, then exit.
            for (const auto& fname : chunks[j]) {
                TString inFile  = fname;
                TString outFile = deriveOutputName(fname);
                processFile(inputDir, inFile, outputDir, outFile, saveMC);
            }
            _exit(0);
        }
    ...
    // Parent: wait for every worker to finish
    ...
    for (pid_t pid : children) {
        ...
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
            anyFailed = true;
            std::cerr << "Worker pid " << pid << " exited abnormally." << std::endl;
        }
    }
