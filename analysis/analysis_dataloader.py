...
class AnalysisDataloader:
    ...
    def setup_MC(self):
        if self.MC is None:
            ...
            return
        self.MC["MC_p"] = np.sqrt(
            self.MC["MC_px"] ** 2 + self.MC["MC_py"] ** 2 + self.MC["MC_pz"] ** 2
        )
        self.pass_truth = (
            (self.MC["MC_W"] > 2) & (self.MC["MC_y"] < 0.85) & (self.MC["MC_Q2"] > 1)
        )

    def setup_reconstructed(self):
        self.pass_reco = self.reconstructed["pass_reco"]
        if "v_z" in self.reconstructed.fields:
            self.reconstructed["vz"] = self.reconstructed["v_z"]
        self.num_events_pass_reco = np.sum(self.reconstructed["pass_reco"])
