...
def main():
    ...
    # fig = plt.figure()
    # plt.hist2d(np.array(MC2_event_arrays["MC"]["MC_x"]), np.array(MC2_event_arrays["MC"]["MC_Q2"]), bins=100, range=((0,1), (1,12)), label="clasdis", norm=mcolors.LogNorm())
    # plt.xlabel("x")
    # plt.ylabel("$Q^2~(GeV^2)$")
    # plt.colorbar()
    # plt.suptitle("clasdis Truth-level phase space")
    # plt.savefig("./clasdis_truth_phasespace.png")

    # fig = plt.figure()
    # plt.hist2d(np.array(MC1_event_arrays["MC"]["MC_x"]), np.array(MC1_event_arrays["MC"]["MC_Q2"]), bins=100, range=((0,1), (1,12)), label="GiBUU", norm=mcolors.LogNorm())
    # plt.xlabel("x")
    # plt.ylabel("$Q^2~(GeV^2)$")
    # plt.colorbar()
    # plt.suptitle("GiBUU Truth-level phase space")
    # plt.savefig("./gibuu_truth_phasespace.png")

    # fig = plt.figure()
    # plt.hist2d(np.array(MC2_event_arrays["MC"]["MC_x"][MC2_pass_truth]), np.array(MC2_event_arrays["MC"]["MC_Q2"][MC2_pass_truth]), bins=100, range=((0,1), (1,12)), label="clasdis", norm=mcolors.LogNorm())
    # plt.xlabel("x")
    # plt.ylabel("$Q^2~(GeV^2)$")
    # plt.colorbar()
    # plt.suptitle("clasdis Truth-level phase space")
    # plt.savefig("./clasdis_truth_phasespace_passtruth.png")

    # fig = plt.figure()
    # plt.hist2d(np.array(MC1_event_arrays["MC"]["MC_x"][MC1_pass_truth]), np.array(MC1_event_arrays["MC"]["MC_Q2"][MC1_pass_truth]), bins=100, range=((0,1), (1,12)), label="GiBUU", norm=mcolors.LogNorm())
    # plt.xlabel("x")
    # plt.ylabel("$Q^2~(GeV^2)$")
    # plt.colorbar()
    # plt.suptitle("GiBUU Truth-level phase space")
    # plt.savefig("./gibuu_truth_phasespace_passtruth.png")

    # fig = plt.figure()
    # plt.hist(np.array(MC1_event_arrays["MC"]["MC_y"]), bins=100, range=(0,1), label="GiBUU", histtype='step', density=True)
    # plt.hist(np.array(MC2_event_arrays["MC"]["MC_y"]), bins=100, range=(0,1), label="clasdis", histtype='step', density=True)
    # plt.legend()
    # plt.xlabel("y")
    # plt.ylabel("Normalized entries")
    # plt.savefig("./gibuu_clasdis_y_distribution.png")

    # fig = plt.figure()
    # plt.hist(np.array(MC1_event_arrays["MC"]["MC_y"][MC1_pass_truth]), bins=100, range=(0,1), label="GiBUU", histtype='step', density=True)
    # plt.hist(np.array(MC2_event_arrays["MC"]["MC_y"][MC2_pass_truth]), bins=100, range=(0,1), label="clasdis", histtype='step', density=True)
    # plt.legend()
    # plt.xlabel("y")
    # plt.ylabel("Normalized entries")
    # plt.savefig("./gibuu_clasdis_y_distribution_passtruth.png")

    variables_to_unfold = ["p", "Q2", "x", "phi_degrees", "theta_degrees"]
    # variables_to_unfold = ["p"]
    ...
    for variable in variables_to_unfold:
        ...
        data_dict_train[variable] = np.array(MC2_event_arrays_train["reconstructed"][variable])
    ...
    data_pass_reco_vector_train = np_to_TVector(MC2_pass_reco_train)
    ...
    if not load_trained_omnifold_model:
        ...
        unbinned_unfolding.SetModelSaveName("GiBUU_clasdis_closure_allobservables_nopasstruth_withpassreco_traintestsplit_5million")
    ...
    for variable in variables_to_unfold:
        ...
        data_dict_test[variable] = np.array(MC2_event_arrays_test["reconstructed"][variable])
    ...
    data_pass_reco_vector_test = np_to_TVector(MC2_pass_reco_test)
    ...
    unbinned_unfolding.SetLoadModelPath("/home/ryan/clas_analysis/clas12-rge-analysis/analysis/GiBUU_clasdis_closure_allobservables_nopasstruth_withpassreco_traintestsplit_5million_iteration_0.pkl")
    ...
    plt.savefig("./gibuu_clasdis_closure_iteration1_step1weights.png")

    unbinned_unfolding.SetTestMCgenDataFrame(df_MCgen_test)
    unbinned_unfolding.SetTestMCrecoDataFrame(df_MCreco_test)
    unbinned_unfolding.SetTestMCPassReco(sim_pass_reco_vector_test)
    unbinned_unfolding.SetLoadModelPath("/home/ryan/clas_analysis/clas12-rge-analysis/analysis/GiBUU_clasdis_closure_allobservables_nopasstruth_withpassreco_traintestsplit_5million_iteration_3.pkl")
    test_unbinned_results = unbinned_unfolding.TestUnbinnedOmnifold()
    ...
    plt.savefig("./gibuu_clasdis_closure_iteration4_step2weights.png")
    ...
    for var in variables_to_unfold:
        ...
        MC2_reco_counts, bin_edges = np.histogram(
            MC2_event_arrays_test["reconstructed"][var][MC2_pass_reco_test],
            bins=num_bins,
            range=(low_bin, high_bin),
        )
        MC2_reco_errors = np.sqrt(MC2_reco_counts)

        total_MC2 = np.sum(MC2_reco_counts)
        norm_MC2_counts = MC2_reco_counts / (total_MC2 * bin_widths)
        norm_MC2_errors = MC2_reco_errors / (total_MC2 * bin_widths)
        ...
        MC1_ratio, MC1_ratio_error = DivideWithErrors(
            norm_MC1_counts, norm_MC1_errors,
            norm_counts, norm_errors,
        )

        MC2_ratio, MC2_ratio_error = DivideWithErrors(
            norm_MC2_counts, norm_MC2_errors,
            norm_counts, norm_errors,
        )

        axs[1].errorbar(
            bin_centers, MC1_ratio, yerr=MC1_ratio_error,
            fmt='o', color="#2ca02c", markersize=7,
            label=f"{MC_names['MC1']}",
        )

        axs[1].errorbar(
            bin_centers, MC2_ratio, yerr=MC2_ratio_error,
            fmt='o', color="#ff7f0e", markersize=7,
        ...
        plt.savefig(f"./gibuu_clasdis_closure_iteration1_{var}_traintestsplit_5million_withratios.png")
        plt.show()
        ...
        plt.tight_layout()
        plt.suptitle(plot_title)
        plt.savefig(f"./gibuu_clasdis_closure_iteration4_{var}_traintestsplit_5million_withratios.png")
        plt.show()
