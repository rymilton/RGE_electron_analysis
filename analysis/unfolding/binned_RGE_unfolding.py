...
for var in variables_to_unfold:
    ...
    MC2gen_hist    = ROOT.TH1D (f"MC2gen_hist_{var}", f"MC2gen_hist_{var}", variable_settings[var]["bins"], variable_settings[var]["range"][0], variable_settings[var]["range"][1])
    MC2reco_hist = ROOT.TH1D (f"MC2reco_hist_{var}", f"MC2reco_hist_{var}", variable_settings[var]["bins"], variable_settings[var]["range"][0], variable_settings[var]["range"][1])
    ...
    for (MCgen, MCreco, pass_reco, pass_truth) in zip(MC2_event_arrays["MC"][f"MC_{var}"], MC2_event_arrays["reconstructed"][f"{var}"], MC2_event_arrays["reconstructed"]["pass_reco"], MC2_pass_truth):
        if not pass_truth:
            continue
        MC2gen_hist.Fill(MCgen)
        if pass_reco:
            MC2reco_hist.Fill(MCreco)
    
    for (measured, pass_reco) in zip(data_event_arrays["reconstructed"][f"{var}"], data_event_arrays["reconstructed"]["pass_reco"]):
        if pass_reco:
    ...
    binned_unfolding_hist.SetTitle("RGE vs. GiBUU C/LD2: Binned UniFold 4 iterations")
    # binned_unfolding_hist.SetTitleSize(0.03)  # default is ~0.05-0.06
    binned_unfolding_hist.SetLineColor(3)
    MC2gen_hist.SetLineColor(2)
    ...
    leg_binned_unfolding.AddEntry(MC2gen_hist, "clasdis Truth","pl")
    leg_binned_unfolding.AddEntry(MCgen_hist, "GiBUU Truth", "pl")
    leg_binned_unfolding.AddEntry(binned_unfolding_hist, "RGE Unfolded")
    leg_binned_unfolding.SetBorderSize(0)
    leg_binned_unfolding.SetFillStyle(0)
    leg_binned_unfolding.Draw()
    c_binned_unfolding.Draw()
    print("Saving hists")
    c_binned_unfolding.SaveAs(f"./RGE_GiBUU_binnedomnifold_1million_{var}_passtruth.pdf")
